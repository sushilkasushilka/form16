...
export function MetricBar({label,value,target,unit,color,icon}){
  ...
  const isFull = p>=100;
  return <div style={{padding:"14px 0",borderTop:`1px solid ${C.border}`}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:8}}>
      <span style={{fontSize:13,color:C.text,fontWeight:500,display:"flex",gap:6,alignItems:"center"}}>{icon&&<span>{icon}</span>}{label}</span>
      <span style={{fontFamily:F.mono,fontSize:13,fontWeight:500,color:isFull?C.accent:C.text,letterSpacing:"0.01em"}}>
        {(value??0).toLocaleString("ru-RU")} <span style={{color:C.muted,fontWeight:500}}>/ {target.toLocaleString("ru-RU")} {unit}</span>
      </span>
    </div>
    <div style={{height:2,background:C.dim,overflow:"hidden"}}>
      <div style={{height:"100%",width:`${p}%`,background:isFull?C.accent:(color||C.text),transition:"width 0.9s cubic-bezier(.16,1,.3,1)"}}/>
    </div>
  </div>;
}
...
export function Btn({children,onClick,variant="primary",disabled,small}){
  const bg = disabled ? C.dim
    : variant==="accent"  ? C.accent
    : variant==="primary" ? C.text
    : variant==="ghost"   ? "transparent"
    : variant==="danger"  ? C.red
    : C.surface;
  const fg = disabled ? C.muted
    : variant==="accent" || variant==="primary" ? C.bg
    : variant==="danger" ? "#fff"
  ...
  return <button onClick={onClick} disabled={disabled} style={{
    ...
    borderRadius:small?999:14,
    padding:small?"9px 18px":"15px 24px",
