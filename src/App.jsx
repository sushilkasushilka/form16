...
// ─── FATSECRET CONNECT (OOB PIN FLOW) ────────────────────────────────────────
function FatSecretConnect({ profile, setProfile, userId }) {
    ...
    return (
      <div style={{background:C.accentDim,border:`1px solid ${C.accent}55`,borderRadius:20,padding:"14px 18px",marginBottom:12,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div><div style={{fontWeight:700,fontSize:13}}>FatSecret</div><div style={{fontSize:11,color:C.accent}}>Подключён — синхронизация активна</div></div>
        </div>
  ...
  return (
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:20,padding:"16px 18px",marginBottom:12}}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
        ...
        <div><div style={{fontWeight:700,fontSize:13}}>FatSecret</div><div style={{fontSize:11,color:C.muted}}>Дневник питания · 2.3 млн российских продуктов</div></div>
      </div>
      ...
      {step==="idle"&&(
        <>
          ...
          <button onClick={startConnect} style={{width:"100%",background:C.accent,color:C.bg,border:"none",borderRadius:14,padding:"11px",fontSize:13,fontWeight:700,fontFamily:"'DM Sans',sans-serif",cursor:"pointer"}}>
      ...
      {step==="pin"&&(
        <>
          <div style={{background:C.surface,borderRadius:14,padding:"12px 14px",marginBottom:12,fontSize:12,color:C.muted,lineHeight:1.7}}>
            ...
            <button onClick={()=>window.open(authorizeUrl,"_blank")} style={{marginTop:8,background:"none",border:`1px solid ${C.accent}`,borderRadius:10,padding:"5px 12px",fontSize:11,color:C.accent,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>
          ...
          <input
            ...
            style={{width:"100%",boxSizing:"border-box",background:C.surface,border:`1.5px solid ${C.border}`,borderRadius:12,padding:"11px 14px",color:C.text,fontSize:15,fontFamily:"'DM Sans',sans-serif",outline:"none",marginBottom:10,textAlign:"center",letterSpacing:4}}
          />
          {errMsg&&<div style={{fontSize:12,color:C.red,marginBottom:8}}>{errMsg}</div>}
          <button onClick={verifyPin} disabled={!pin.trim()} style={{width:"100%",background:pin.trim()?C.accent:C.dim,color:pin.trim()?C.bg:C.muted,border:"none",borderRadius:14,padding:"11px",fontSize:13,fontWeight:700,fontFamily:"'DM Sans',sans-serif",cursor:pin.trim()?"pointer":"default"}}>
            Подтвердить
          </button>
        </>
      )}
      ...
      {step==="success"&&(
        <div style={{textAlign:"center",padding:"12px 0"}}>
          ...
          <div style={{fontSize:14,fontWeight:700,color:C.accent}}>FatSecret подключён!</div>
      ...
      {step==="error"&&(
        <div>
          ...
          <button onClick={()=>setStep("idle")} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"8px 16px",fontSize:12,color:C.muted,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Попробовать снова</button>
        </div>
      )}
    </div>
  );
